using EQX.UI.Language;
using Microsoft.Extensions.DependencyInjection;
using Microsoft.Extensions.Hosting;

namespace EQX.TemplateProject.Extensions
{
    public static class AddLanguageServiceExtension
    {
        public static IHostBuilder AddLanguageService(this IHostBuilder hostBuilder)
        {
            hostBuilder.ConfigureServices((hostContext, services) =>
            {
                services.AddSingleton<ILanguageService>(s => {
                    IEnumerable<ILanguageDefinition> supportedLanguages = new List<ILanguageDefinition>
                    {
                        new VietnameseLanguage($"/EQX.TemplateProject;component/Resource/Language/VietnameseLanguage.xaml"),
                        new EnglishLanguage($"/EQX.TemplateProject;component/Resource/Language/EnglishLanguage.xaml"),
                        new KoreanLanguage($"/EQX.TemplateProject;component/Resource/Language/KoreanLanguage.xaml")
                    };

                    return new LanguageService(supportedLanguages);
                });
            });

            return hostBuilder;
        }
    }
}
